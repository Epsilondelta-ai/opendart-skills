...
class OpenDartClient:
    ...
    def require_api_key(self) -> str:
        if not self.api_key:
            raise ValueError("OPENDART_API_KEY is required; live smoke lane is opt-in and skipped by default when absent.")
        return self.api_key
    ...
    def _request_bytes(self, path: str, params: dict[str, object | None]) -> bytes:
        request = Request(
            ...
            headers={"User-Agent": "opendart-skills/1.0"},
        )
    ...
    def fetch_corp_codes(self) -> list[CorpRecord]:
        ...
        corp_code_xml_path(self.cache_dir).write_text(
            f"<result>\n{xml_body}\n</result>\n",
            encoding="utf-8",
        )
    ...
    def company_overview(self, corp_code: str) -> dict:
        spec = get_endpoint("company_overview")
        return self.request_json(spec.path, corp_code=corp_code)

    def disclosure_list(self, **params: object | None) -> dict:
        spec = get_endpoint("disclosure_list")
